...
# ---------------------------------------------------------------------------
# Subject-level train/val partition (prevents visit-level leakage).
# ADNI has multiple visits per subject; a random per-sample split would place
# different visits of the same subject in both train and val, leaking identity.
# We split on unique SubjectID instead and reuse the SAME partition for both the
# ViT fine-tuning stage and the CNN stage, so val subjects are never seen by
# either. `df` is built from `df_adni` in the same row order, so positional
# indices line up with the datasets below.
...
# Split Dataset (subject-level; same partition as the CNN stage below)
generator = torch.Generator().manual_seed(universal_seed)
vit_train_dataset = torch.utils.data.Subset(vit_dataset, subject_train_indices)
vit_val_dataset = torch.utils.data.Subset(vit_dataset, subject_val_indices)
...
# freshly-initialized classification head fully trainable. After training the
# adapters are merged back (merge_and_unload) so the feature-extraction code
# below can keep calling model.vit(...) unchanged.
...
# for i, path in enumerate(tqdm(expanded_image_paths[start_idx:], 
#                               total=len(expanded_image_paths) - start_idx, 
#                               desc=f"Scanning .pt files from {start_idx}")):
#     real_idx = i + start_idx  # adjust index back to original
#     try:
#         _ = torch.load(path, map_location="cpu")
#     except Exception as e:
#         print(f"\n❌ Corrupt file at index {real_idx}: {path}")
...
# Merge DoRA adapters into the base ViT so the feature-extraction code below can
# keep using model.vit(...) as before.
...
os.makedirs(f"../adni_storage/ADNI_features/train_adni_e{vit_train_epochs}_s{slice_count}/", exist_ok=True)
...
for _, row in tqdm(df_adni.iterrows(), total=len(df_adni), desc="Processing images"):
    ...
    feature_file_path = f"../adni_storage/ADNI_features/train_adni_e{vit_train_epochs}_s{slice_count}/{image_title}_features.npy"
    if os.path.exists(feature_file_path):
        ...
        # Save the image
        # img.save(f"../adni_storage/ADNI_features/train_adni_e{vit_train_epochs}_s{slice_count}/featuremaps/{image_title}_fm.png")
...
# Subject-level split: reuse the exact partition from the ViT stage so no
# subject's visits leak across train/val. Requires 1:1 alignment between the
# feature list and df (every row processed, no skips).
assert len(dataset) == len(df), (
    f"dataset/df misalignment ({len(dataset)} vs {len(df)}); subject-index "
    "mapping would be invalid. Some feature files were likely skipped.")
...
# ---------------------------------------------------------------------------
# After training: reload the BEST checkpoint (lowest val loss seen during
# training) and rewrite its predictions, so the downstream age-gap analysis
# uses the best model rather than whatever the last epoch happened to be.
# ---------------------------------------------------------------------------
print("Reloading best model to write final (best-model) predictions...")
with open(f"model_dumps/mix/{sys.argv[1]}_best_model_with_metadata.pkl", "rb") as f:
...
def _write_best_predictions(loader, indices, out_csv):
    outs = {}
    with torch.no_grad():
        for idx, (features, sex, age) in enumerate(loader):
            features = features.unsqueeze(1).to(device)
            sex = sex.to(device)
            outputs = model(features, sex)
            for i in range(outputs.size(0)):
                outs[indices[idx * batch_size + i]] = outputs[i].item()
    max_index = max(outs.keys())
    dfp = pd.DataFrame(index=range(max_index + 1), columns=["Predicted_Age"])
    for index, value in outs.items():
        dfp.loc[index, "Predicted_Age"] = value
    d = df.copy()
    d['Predicted_Age'] = dfp['Predicted_Age']
    d.loc[outs.keys()].to_csv(out_csv)


_write_best_predictions(val_loader, val_indices, f"model_dumps/mix/{sys.argv[1]}_predicted_ages_val.csv")
_write_best_predictions(train_loader, train_indices, f"model_dumps/mix/{sys.argv[1]}_predicted_ages_train.csv")
print(f"Best-model predictions written (best val loss {best_ckpt['loss']:.4f}).")
