...
metadata_path = "../abide_storage/abide_brainrotnet_metadata_asd.csv"
...
df_abide = df_abide.sort_values(by='Age', ascending=True).reset_index(drop=True)
df_abide = df_abide.head(1053)
...
os.makedirs(f"../abide_storage/ABIDEII_features/train_abide_e{vit_train_epochs}_s{slice_count}/", exist_ok=True)
for _, row in tqdm(df_abide.iterrows(), total=len(df_abide), desc="Processing test images"):
    ...
    feature_file_path = f"../abide_storage/ABIDEII_features/train_abide_e{vit_train_epochs}_s{slice_count}/{image_title}_features.npy"
    # feature_file_path = f"ixi_storage/IXI_features/train_e{vit_train_epochs}_s{slice_count}/{image_title}_features.npy"
        ...
        if os.path.exists(filepath):
            try:
                ...
                for slice_idx in range(data.shape[0]):
                    ...
                    slice_data = (slice_data - np.min(slice_data)) / (np.max(slice_data) - np.min(slice_data) + 0.000000001)  # Normalize
